...
public class HaxeIndentProcessor {
  ...
  public Indent getChildIndent(ASTNode node) {
    ...
    final int braceStyle = HaxeTokenTypeSets.FUNCTION_DEFINITION.contains(superParentType) ? settings.METHOD_BRACE_STYLE : settings.BRACE_STYLE;

    if (parent == null || parent.getTreeParent() == null) {
      return Indent.getNoneIndent();
    }
    if (HaxeTokenTypeSets.COMMENTS.contains(elementType)) {
    ...
    if (HaxeTokenTypeSets.FUNCTION_DEFINITION.contains(parentType) || parentType == CALL_EXPRESSION) {
