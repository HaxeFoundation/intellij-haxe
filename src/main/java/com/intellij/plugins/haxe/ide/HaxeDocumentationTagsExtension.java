...
public class HaxeDocumentationTagsExtension implements Parser.ParserExtension {
    ...
    private HaxeDocumentationTagsExtension() {
    }
...
class HaxeDocumentationTagsVisitor extends AbstractVisitor {
    ...
    private static void processSinceTag(Text text, String tag, String content) {
    ...
    private static void processSeeTag(Text text, String tag, String content) {
    ...
    private static void processParamsTag(Text text, String tag, String content) {
    ...
    private static void processEventTag(Text text, String tag, String content) {
        TableBlock tableBlock = new TableBlock();
        text.insertBefore(tableBlock);

        // TODO  bundle ?
        createTableHeader(tableBlock, "Event", "Description");
        lookForAndAddEventTagsToTable(text, tableBlock);

        text.unlink();
    }

    private static void highlightTag(Text text, String tag, String content) {
    ...
    private static void lookForAndAddParameterTagsToTable(Text first, TableBlock tableBlock) {
        ...
        while (next instanceof Text text) {
            ...
            if (matcher.find()) {
                ...
                if (TAG_RETURN.equalsIgnoreCase(tag)) {
                    next = findNextTagTextNode(text, null);
    ...
    private static void lookForAndAddEventTagsToTable(Text first, TableBlock tableBlock) {
        Node next = first;
        while (next instanceof Text text) {
            Matcher matcher = docTagPattern.matcher(text.getLiteral());
            if (matcher.find()) {
                String tag = matcher.group(1);
                String content = matcher.group(2);
                if (TAG_EVENT.equalsIgnoreCase(tag)) {
                    next = findNextTagTextNode(text, TAG_EVENT);
                    TableRow row = findAndCreateParameterRoEventRow(text, content);
                    tableBlock.appendChild(row);
                    text.unlink();
                } else {
    ...
    private static void lookForAndAddSeeTagsToList(BulletList list, Node node) {
        Node next = node;
        while (next instanceof Text text) {
            Matcher matcher = docTagPattern.matcher(text.getLiteral());
            if (matcher.find()) {
    ...
    private static @NotNull TableRow findAndCreateReturnRow(Text text, String content) {
        ...
        while (nextContentNode != null
               ...
               && !(nextContentNode instanceof SoftLineBreak)
               && !(nextContentNode instanceof Paragraph)
        ) {
    ...
    private static @NotNull TableRow findAndCreateParameterRoEventRow(Text text, String content) {
        TableRow parameterRow = new TableRow();
...
//                   && !(nextContentNode instanceof SoftLineBreak)
    ...
    private static @NotNull ListItem findAndCreateListItem(Text text, String content, boolean blank) {
