...
public class HaxeUnaryExpressionAnnotator implements Annotator {
    ...
    private void checkPostfixExpression(@NotNull HaxePostfixExpression postfixExpression, @NotNull AnnotationHolder holder) {
        // ignore if  inside meta, ex @:op(A++)
        if (postfixExpression.getParent() instanceof HaxeCompiletimeMetaArg) return;

        HaxeAssignableOperator operator = postfixExpression.getAssignableOperator();
        ...
        if (operator.textMatches("++") || operator.textMatches("--")) {
            ...
            if (expression instanceof HaxeLiteralExpression || expression instanceof HaxeStringLiteralExpression) {
                holder.newAnnotation(HighlightSeverity.ERROR, "Invalid assign").range(postfixExpression).create();
                return;
            }
            ...
            if (expression instanceof HaxeArrayAccessExpression) {
                ...
                checkIfPostfixIsValid(postfixExpression, holder, result);
                return;
            ...
            if (expression instanceof HaxeReferenceExpression referenceExpression) {
                ...
                if (nonRealVariableWrite) {
                    holder.newAnnotation(HighlightSeverity.ERROR, "This field cannot be accessed because it is not a real variable").range(postfixExpression).create();
                    return;
                }
                boolean isWritable = checkIfPropertyWritable(referenceExpression);
                if (!isWritable) {
                    holder.newAnnotation(HighlightSeverity.ERROR, "This expression cannot be accessed for writing").range(postfixExpression).create();
                    return;
                }
    ...
    private static void checkIfPostfixIsValid(@NotNull HaxePostfixExpression postfixExpression, @NotNull AnnotationHolder holder, @NotNull ResultHolder result) {
        if (result.isUnknown()) return;

        SpecificTypeReference type = result.getType();

        if (result.isImmutable()) {
            holder.newAnnotation(HighlightSeverity.ERROR, "Cannot assign to immutable reference").range(postfixExpression).create();
        } else if (!type.isAbstractType()) {
            holder.newAnnotation(HighlightSeverity.ERROR, type.toPresentationString() + " should be Int").range(postfixExpression).create();
        } else {
            ...
            //TODO mlo: if abstract check operator overrides
        }
    }
