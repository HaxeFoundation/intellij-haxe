...
public class HaxeFoldingBuilder implements FoldingBuilder {
  ...
  private static boolean isDocComment(IElementType type) {
    return type == HaxeTokenTypeSets.DOC_COMMENT;
  }
  ...
  private static boolean isBodyBlock(IElementType elementType) {
    return HaxeTokenTypeSets.CLASS_BODY_TYPES.contains(elementType);
  }

  private static boolean isComment(IElementType elementType, ASTNode node) {
    return HaxeTokenTypeSets.ONLY_COMMENTS.contains(elementType);
  }

  private static boolean isCompilerConditional(IElementType elementType) {
    return HaxeTokenTypeSets.ONLY_CC_DIRECTIVES.contains(elementType);
  }
  ...
  private static String stripComment(IElementType elementType, String text) {
    ...
    if (elementType == HaxeTokenTypeSets.DOC_COMMENT) {
      text = HaxeStringUtil.terminateAt(text, '\n');
  ...
  private static String getCCPlaceholder(ASTNode node) {
    ...
    while (null != next && next.getElementType() == HaxeTokenTypeSets.PPEXPRESSION) {
  ...
  private boolean isUnusedCCRegion(ASTNode node) {
    ...
    if (!HaxeTokenTypeSets.ONLY_CC_DIRECTIVES.contains(node.getElementType())) {
      return false;
    }

    node = node.getTreeNext();
    while (HaxeTokenTypeSets.PPEXPRESSION == node.getElementType()) {
      node = node.getTreeNext();
    }
    return node.getElementType() == HaxeTokenTypeSets.PPBODY;
