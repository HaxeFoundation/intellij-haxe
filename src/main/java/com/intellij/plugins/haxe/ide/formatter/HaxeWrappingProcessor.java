...
public class HaxeWrappingProcessor {
  ...
  Wrap createChildWrap(ASTNode child, Wrap defaultWrap, Wrap childWrap) {
    ...
    if (elementType == PARAMETER_LIST || elementType == EXPRESSION_LIST) {
      ...
      if (HaxeTokenTypeSets.FUNCTION_DEFINITION.contains(parentType) &&
    ...
    //Binary expressions
    //
    if (HaxeTokenTypeSets.BINARY_EXPRESSIONS.contains(elementType) && mySettings.BINARY_OPERATION_WRAP != CommonCodeStyleSettings.DO_NOT_WRAP) {
      if ((mySettings.BINARY_OPERATION_SIGN_ON_NEXT_LINE && HaxeTokenTypeSets.BINARY_OPERATORS.contains(childType)) ||
