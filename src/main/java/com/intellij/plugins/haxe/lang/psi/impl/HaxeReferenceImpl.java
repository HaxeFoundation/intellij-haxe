...
abstract public class HaxeReferenceImpl extends HaxeExpressionImpl implements HaxeReference {
  ...
  private boolean isType(Class clazz) {
    if (clazz.isInstance(this)) {
      if (log.isTraceEnabled()) log.trace(HaxeDebugUtil.getCallerStackFrame() + "Resolving " + this.getDebugName() + " as class type " + clazz.getName());
      return true;
    }
    if (log.isTraceEnabled()) log.trace(HaxeDebugUtil.getCallerStackFrame()+ this.getDebugName() + " is not a " + clazz.getName());
    return false;
  }
  ...
  private boolean isType(Object o, Class clazz) {
    if (clazz.isInstance(o)) {
      if (log.isTraceEnabled()) log.trace(HaxeDebugUtil.getCallerStackFrame() + "Resolving " + o + " as class type " + clazz.getName());
      return true;
    }
    if (log.isTraceEnabled()) log.trace(HaxeDebugUtil.getCallerStackFrame() + ""+ o + " is not a " + clazz.getName());
    return false;
  }
