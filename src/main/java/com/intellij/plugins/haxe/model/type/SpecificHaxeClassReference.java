...
public class SpecificHaxeClassReference extends SpecificTypeReference {
  ...
  private void emptyCollectionAssignment(Compatibility direction, Set<SpecificHaxeClassReference> list) {
    if (direction == Compatibility.ASSIGNABLE_TO && context instanceof HaxeArrayLiteral && null == ((HaxeArrayLiteral)context).getExpressionList()) {
      ResultHolder unknownHolderKey = getUnknown(context).createHolder();
      ResultHolder unknownHolderValue = getDynamic(context).createHolder();
      SpecificHaxeClassReference holder = (SpecificHaxeClassReference)createMap(unknownHolderKey, unknownHolderValue, context);
      list.add(holder);
    }
  }
  private void literalCollectionAssignment(Compatibility direction, Set<SpecificHaxeClassReference> list) {
    ...
    if (direction == Compatibility.ASSIGNABLE_TO) {
      if (isLiteralArray()) {
        ResultHolder unknownHolder = getAny(context).createHolder();;
        SpecificHaxeClassReference array = (SpecificHaxeClassReference)createArray(unknownHolder, context);
        list.add(array);
      }
      if (isLiteralMap()) {
        ResultHolder[] specifics = this.getSpecifics();

        ResultHolder unknownHolderKey = getAny(context).createHolder();
        ResultHolder unknownHolderValue = getAny(context).createHolder();

        SpecificHaxeClassReference anyAnyMap = (SpecificHaxeClassReference)createMap(unknownHolderKey, unknownHolderValue, context);
        SpecificHaxeClassReference xAnymap = (SpecificHaxeClassReference)createMap(specifics[0], unknownHolderValue, context);
        SpecificHaxeClassReference anyXmap = (SpecificHaxeClassReference)createMap(unknownHolderKey, specifics[1], context);
        list.add(anyAnyMap);
        list.add(xAnymap);
        list.add(anyXmap);
      }
