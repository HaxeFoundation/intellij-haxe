...
public class HaxeTypeUnifier {
  ...
  static public SpecificTypeReference unifyFunctions(SpecificFunctionReference a,
    ...
    final List<SpecificFunctionReference.Argument> pa = a.getArguments();
    final List<SpecificFunctionReference.Argument> pb = b.getArguments();
    if (pa.size() != pb.size()) return SpecificTypeReference.getInvalid(a.getElementContext());
    final ArrayList<SpecificFunctionReference.Argument> arguments = new ArrayList<>();

    int size = pa.size();
    for (int n = 0; n < size; n++) {
      final SpecificFunctionReference.Argument unifiedArgument = unify(pa.get(n), pb.get(n));
  ...
  @NotNull
  private static SpecificFunctionReference.Argument unify(SpecificFunctionReference.Argument a, SpecificFunctionReference.Argument b) {
    if (a.isOptional() != b.isOptional()) {
      ResultHolder invalidType = SpecificTypeReference.getInvalid(a.getType().getElementContext()).createHolder();
      return new SpecificFunctionReference.Argument(a.getIndex(), a.isOptional(), invalidType, a.getName());
    }

    return new SpecificFunctionReference.Argument(a.getIndex(), a.isOptional(), unify(a.getType(), b.getType()), a.getName());
  }
